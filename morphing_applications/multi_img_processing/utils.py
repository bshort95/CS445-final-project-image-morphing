...
def warp_image_affine_transform_multiple_imgs_laplacian_pyramid_blending(no_of_intermed, imgs,tris):
    # n=no_of_intermed+2
    n=no_of_intermed
    base_img = imgs[0]
    frame_count = 0
    ...
    for x in range(len(imgs)-1):
        ...
        for k in range(1,no_of_intermed+1):
            ...
            # name="./generated-images/multi-input-linear-dissolve/inter_"+str(k)+".jpg"
#           cv2.imshow("inter"+str(k),inter)
            # name="./morphing_applications/multi_img_processing/multi-input-generated-imgs/inter_"+str(frame_count)+".jpg"
            name="./generated-images/multi-input-laplacian-pyramid-blending/inter_"+str(frame_count)+".jpg"
            cv2.imwrite(name, inter) 
            frame_count = frame_count + 1
       # cv2.waitKey(0)
       # cv2.destroyAllWindows()
